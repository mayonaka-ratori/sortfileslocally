...
NOTE: This test may fail with an ImportError (DLL load failed) on Windows 
environments missing the necessary C++ Redistributable for onnxruntime.
If that occurs, the test will gracefully skip the video processing phase.
"""
import os
import sys
import json
...
def create_test_video(out_path: str, duration: int = 5):
    ...
    try:
        ...
        print("WARNING: ffmpeg subprocess timed out")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  [SKIP] Video tests skipped due to environment dependency: {e}\n")
