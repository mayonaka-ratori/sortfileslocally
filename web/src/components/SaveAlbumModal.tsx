...
export default function SaveAlbumModal({ isOpen, onClose, currentQuery, currentFilters }: SaveAlbumModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" role="dialog" aria-modal="true" aria-labelledby="save-album-title">
            <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-6">
                    <h2 id="save-album-title" className="text-xl font-semibold text-white mb-2">{t('title')}</h2>
